...
const SECRET="sjfdhgjsrghsjg"
const bcrypt = require('bcryptjs');
const jwt = require('jsonwebtoken');
const URL = "mongodb+srv://murran171:Muralitharan171@cluster0.t7ewgl7.mongodb.net/";
...
app.get('/', function(req,res){
    res.send("Welcome to CRM Project !")
})
...
app.post('/adminlogin',async function(req,res){
    try {
        ...
        if (admin) {
            ...
            if (compare) {
                const token = jwt.sign({_id: admin._id},SECRET,{expiresIn:"10m"})
                res.json({token})
            } else {
                console.log(error)
                res.json({message:"Enter correct Password"})
